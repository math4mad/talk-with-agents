---
title: "下一步：在一族函数空间里搜索「谁生成了数据」"
subtitle: "SARCOS 逆动力学项目的第一次追问（对话记录）"
description: "与 Agent 的追问：不预设假设空间，而是准备一族由基函数张成的候选空间，用边际似然、LOO-CV 与贝叶斯模型平均去搜索最接近数据生成机制的那一个。"
author:
  - name: Math4Mad
    url: "https://github.com/math4mad"
date: "2026-09-06"
bibliography: ../../references.bib
image: "../../assets/figures/thumb-spaceladder.png"
format:
  html:
    toc-depth: 3
---
...
这是 [SARCOS inverse dynamics with Agents](sarcos-ml-with-agents.qmd) 之后的第一次追问，
原始记录：`Talkmd/昨天在 llm 帮助下实现了这个项目 [SARCOS]md.md`。
候选空间、边际似然与 LOO 的完整推导见 [@rasmussen2006gpml]，谱混合核那一格对应 [@wilson2013sm]。
文中提出的搜索流程，已经在
[代码片段：在候选函数空间中搜索「谁生成了数据」](../snippets/function-space-search.qmd)
里用合成数据跑通了一遍——那一页有真实输出，本页保留对话原样。
:::
...
> 昨天在 llm 帮助下实现了这个项目 [SARCOS inverse dynamics with scikit-learn, GPyTorch and PyStan](https://math4mad.github.io/SARCOS-ML-with-Agents/)。今天我想继续沿着昨天的思路，在这个数据集的基础上探讨一下：在一序列不同基函数张成的空间集合里，搜索出最接近生成数据模型的那个空间。
:::
...
上面第 6 节里那段 `space_search.py` 草稿已经不是草稿：
[代码片段：在候选函数空间中搜索「谁生成了数据」](../snippets/function-space-search.qmd)
用合成数据把 H1–H7 全部跑了一遍，边际似然的排序确实指向了包含真值的那个最小空间。
